ПРОГНОЗУВАННЯ ОФОРМЛЕННЯ ДЕПОЗИТІВ КЛІЄНТАМИ БАНКУ
 Bank Marketing Dataset — Binary Classification
...
RANDOM_STATE = 42
TEST_SIZE = 0.2
VAL_SIZE = 0.15           для підбору порогу та eval_set в hyperopt
DECISION_THRESHOLD = 0.6  початкове значення, підбирається на val set
...
def distribution_plots(dataset, idx):
    """
    Гістограми розподілу цільової змінної для переданих колонок.
    Параметри:
        dataset : pd.DataFrame
        idx     : list — перелік колонок для побудови графіків
    """
    nrows = len(idx)
    fig, axes = plt.subplots(nrows=nrows, ncols=1, figsize=(6, 4 * nrows))

     Якщо передано лише одну колонку — обгортаємо у список,
     щоб уникнути помилки при індексації axes[i]
    if nrows == 1:
        axes = [axes]
...
def outliers(columns, dataset):
    """
    Розраховує кількість викидів за IQR для кожної числової колонки.
    pdays=999 виключається з розрахунку як sentinel-значення.
    """
    outlier_list = []
    for col in columns:
...
print(outlier_stats)
 ...
 Кліпінг викидів до верхньої межі IQR
 pdays: межа ~13, campaign: ~6, age: ~69
for _, row in outlier_stats.iterrows():
    col = row['col']
    if col == 'pdays':
         Кліпуємо лише не-sentinel значення
        mask = train[col] < 999
...
def outlier_plots(dataset, out_cols):
    """
    Scatterplot кожної колонки з позначкою верхньої межі IQR.
    Виправлено: вісь Y фіксована як 'y' (цільова), вісь X — поточна колонка.
    """
...
print(train.describe())
 ...
 5. FEATURE ENGINEERING  (виконується після аналізу, до split)
...
education_order = ['unknown', 'illiterate', 'basic.4y', 'high.school',
 ...
 Спочатку відокремлюємо test (20%), потім val із решти (15% від загального)
...
print(f"Train: {X_train.shape}, Val: {X_val.shape}, Test: {X_test.shape}")
 ...
 7. ПРЕПРОЦЕСОР (один об'єкт, fit лише на train)
...
def evaluate(model, X_tr, X_val, y_tr, y_val, label=''):
    """
    Виводить ROC-AUC та classification report для train і val.
...
def best_threshold(model, X_val, y_val):
    """
    Підбирає поріг класифікації на val set за максимальним F1.
    """
...
results = []   збираємо метрики всіх моделей для фінального порівняння


 9. МОДЕЛЬ 1 — Логістична регресія (pipeline)
...
results.append(evaluate(logreg_pipeline, X_train, X_val, y_train, y_val,
 ...
 Підбір порогу на VAL set (не на test!)
...
print(f"\nПомилкові передбачення: {(test_pred_logreg['y'] != test_pred_logreg['pred']).sum()}")
 ...
 10. SMOTENC — підготовка для моделей без вбудованого балансування

 Fit preprocessor один раз на train, transform train/val/test
...
X_test_proc = pd.DataFrame(
 ...
 SMOTENC приймає індекси категоріальних ознак (після OneHot всі вони бінарні)
 Визначаємо булеву маску: True для onehot + ordinal колонок
...
print(f"Після SMOTENC: {X_train_sm.shape}, розподіл y: {pd.Series(y_train_sm).value_counts().to_dict()}")
 ...
 11. МОДЕЛЬ 2 — Логістична регресія зі SMOTENC
...
for label, mask in [
 ...
 Feature importance для LogReg+SMOTENC
...
def search_model(search, X_tr, y_tr):
    """GridSearch/RandomizedSearch з виводом найкращих параметрів."""
...
plt.show()
 ...
 14. МОДЕЛЬ 5 — LightGBM з нативною підтримкою категоріальних ознак

 Для LGBM використовуємо оригінальні (не preprocessed) дані з category dtype
...
for col in cat_cols_lgb:
 ...
 SMOTENC на оригінальних даних (до preprocessingu)
...
X_tr_lgb_sm, y_tr_lgb_sm = smotenc_lgb.fit_resample(X_train_lgb, y_train)
 ...
 Відновлюємо category dtype після SMOTENC (може скидати)
for col in cat_cols_lgb:
...
def objective(params):
    clf = lgb.LGBMClassifier(
     ...
     eval_set використовує VAL (не test) для early stopping
...
results.append({
 ...
 15. ФІНАЛЬНЕ ПОРІВНЯННЯ МОДЕЛЕЙ
...
print(results_df[['label', 'auc_train', 'auc_val', 'f1_val']].to_string(index=False))
 ...
 16. ВІЗУАЛІЗАЦІЯ КАТЕГОРІАЛЬНИХ ЗМІННИХ (EDA)


def custom_subplots(dataset, target_col='y'):
    """
    Інтерактивні графіки розподілу категоріальних змінних за цільовою колонкою.
    Об'єднано custom_subplots() та custom_subplots_pred() через параметр target_col.
    """
...
pdays_grp = (train[train['pdays'] < 999]
 ...
 17. ДОПОМІЖНІЙ АНАЛІЗ (EDA — пропорції категоріальних змінних)
